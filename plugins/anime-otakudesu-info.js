...
let handler = async (m, { conn, usedPrefix, args }) => {
   ...
   let res = (await fetch('https://pnggilajacn.my.id/api/anime/otakudesu/info', { link: args[0] } )).data;
    if (res.status != 200) throw res.message;
    if (!res) throw res.message;
    let v = res.result
	let arr = []
	for (let x of v.episode) arr.push({ title: x._title, rowId: `${usedPrefix}otakudl ${x._eps}` })
