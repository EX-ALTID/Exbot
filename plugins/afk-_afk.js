...
export function before(m,{usedPrefix}) {
  ...
  *${conn.getName(m.sender)}* @${m.sender.split("@")[0]} *berhenti AFK* ${user.afkReason ? ' setelah ' + user.afkReason : ''}
  ...
  let kataafk = ['mau turu', 'mau nyolong', 'Ke rumah ayang', 'jagain lilin', 'beli pop es', 'kawin lari', 'main kelereng', 'petak umpet', 'push renk', 'push up joni', 'olahraga', 'onani', 'beraq', 'open bo', 'di suruh emak', 'kerja']
    conn.sendButton(m.chat, caption, wm, null, [['AFK Lagi', `${usedPrefix}afk` + kataafk.getRandom()]], m, { mentions: conn.parseMention(caption) })
        user.afk = -1
        user.afkReason = ''
    }
    let jids = [...new Set([...(m.mentionedJid || []), ...(m.quoted ? [m.quoted.sender] : [])])]
  ...
  *Jangan tag* *${conn.getName(jid)}* @${jid.split("@")[0]}!
  Dia sedang AFK *${reason ? ' alasanya' + reason : 'tanpa alasan'}*
  Selama ${(new Date - afkTime).toTimeString()}
  `.trim()
    conn.sendButton(m.chat, caption, wm, null, [['Berhenti', `${usedPrefix}tts id kok berhenti`]], m, { mentions: conn.parseMention(caption) })
    }
    return true
}
