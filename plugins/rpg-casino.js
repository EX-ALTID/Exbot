...
let imgr = flaaa.getRandom()
    ...
    conn.sendButton(m.chat, caption, wm, imgr + 'LOSE', [['Try Again ' + args[0], `${usedPrefix}casino ` + args[0]]], m, { mentions: conn.parseMention(caption) })
    } else if (Aku < Kamu) {
    let caption = `                💰 *C A S I N O* 💰\n\n${htjava} *@${m.sender.split("@")[0]}* - [USER]\n┗┅⭑ ${Kamu} Point\n${htjava} *@${nomorbot.split("@")[0]}* - [BOT]\n┗┅⭑ ${Aku} Point\n\n🎉 *WIN* 🎉\nKamu mendapatkan ${count * 2} Uang(xp)`.trim()
    conn.sendButton(m.chat, caption, wm, imgr + 'WIN', [['Try Again ' + args[0], `${usedPrefix}casino ` + args[0]]], m, { mentions: conn.parseMention(caption) })
    } else {
    let caption = `                💰 *C A S I N O* 💰\n\n${htjava} *@${m.sender.split("@")[0]}* - [USER]\n┗┅⭑ ${Kamu} Point\n${htjava} *@${nomorbot.split("@")[0]}* - [BOT]\n┗┅⭑ ${Aku} Point\n\n🔖*DRAW* 🔖\nKamu mendapatkan ${count * 1} Uang(xp)`.trim()
    conn.sendButton(m.chat, caption, wm, imgr + 'DRAW', [['Try Again ' + args[0], `${usedPrefix}casino ` + args[0]]], m, { mentions: conn.parseMention(caption) })
            }
