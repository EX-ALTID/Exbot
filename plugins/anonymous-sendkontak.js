...
async function handler(m, { usedPrefix, command, conn, text }) {
...
➔ Nama: ${name}`
	...
	if (other) this.sendHydrated(other, `${htki} ᴀɴᴏɴʏᴍᴏᴜs ᴄʜᴀᴛs ${htka}`, tks, await conn.profilePictureUrl(m.sender, 'image').catch(_ => './src/avatar_contact.png'), `wa.me/${m.sender.split`@`[0]}`, 'ᴛᴜʀɴ ᴄʜᴀᴛ ᴘᴀʀᴛɴᴇʀ', null,null, [['ʟᴇᴀᴠᴇ', `${usedPrefix}leave`],[null,null],[null,null]], 0,  { mentionedJid: [m.sender]})
}
